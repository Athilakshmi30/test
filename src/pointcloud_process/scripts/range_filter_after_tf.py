...
def reconstructedCallBack(msg):
    global pc2_roi,pc2_roi_nocolour,data_flag
    if(not data_flag):
        print("im in")
        ...
        for data in pc2:
            ...
            points_list.append([x-0.034, y   , z ,data[3]])  #0.034
...
def restartNode():
    ...
    pc2_roi_nocolour = PointCloud2()
    data_flag = False
    if(rospy.has_param('axalta/ccscore/dashboard/restart_range_filter_after_tf_node_trigger')):
        rospy.set_param('axalta/ccscore/dashboard/restart_range_filter_after_tf_node_trigger',False)
        
        time.sleep(1)
        rospy.set_param('axalta/ccscore/dashboard/restart_reorient_wrong_normals_trigger',True)
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if(rospy.has_param('axalta/ccscore/dashboard/restart_range_filter_after_tf_node_trigger') and rospy.get_param('axalta/ccscore/dashboard/restart_range_filter_after_tf_node_trigger')):
            print("-------------------Restarting range filter after tf node-------------------")
            restartNode()
