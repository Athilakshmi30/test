...
class PathPlanning():
    ...
    def two_d_curve_fit(self, data_list, z_val, horizontal=False):
        ...
        for ind, i in enumerate(new_fitted_points):
            ...
            elif self.diff_dx_dy(prev_x,prev_y,i[0],i[1])>= 0.143:# and self.diff_dx_dy(prev_x,prev_y,i[0],i[1])< 0.18: #abs(prev_x - i[0]) >= 0.143:   # 0.02
                #print("Second condition")
                prev_x = i[0]
                prev_y = i[1]
                drop_new_fitted_points.append(i)
        ...
        for ind, i in enumerate(prev_new_fitted_points):
            ...
            elif self.diff_dx_dy(prev_x_old,prev_y_old,i[0],i[1])>= 0.143:# and self.diff_dx_dy(prev_x_old,prev_y_old,i[0],i[1])< 0.18: #abs(prev_x_old - i[0]) >= 0.143:  # 0.02
    ...
    def get_diffy_diffz(self, dy_dz_change_rows):
        ...
        print("-------------------------")
        #time.sleep(5)
        diff_dy_change = abs(float(
            dy_change_row_avg[1]-dy_change_row_avg[0]))
        diff_dz_change = abs(float(
