...
class PathPlanning():
    ...
    def __init__(self):
        ...
        self.left_right_index_offset = 0.0762 #0.0762  # 3 inches #0.1524 - 6 inches
        ...
        self.left_right_index_shifitng_factor = 2
    ...
    def two_d_curve_fit(self, data_list, z_val, horizontal=False):
        ...
        for ind, i in enumerate(new_fitted_points):
            ...
            elif self.diff_dx_dy(prev_x,prev_y,i[0],i[1])>= 0.001:# and self.diff_dx_dy(prev_x,prev_y,i[0],i[1])< 0.18: #abs(prev_x - i[0]) >= 0.143:   # 0.02
                #print("Second condition")
                prev_x = i[0]
                prev_y = i[1]
                drop_new_fitted_points.append(i)
        ...
        for ind, i in enumerate(prev_new_fitted_points):
            ...
            elif self.diff_dx_dy(prev_x_old,prev_y_old,i[0],i[1])>= 0.001:# and self.diff_dx_dy(prev_x_old,prev_y_old,i[0],i[1])< 0.18: #abs(prev_x_old - i[0]) >= 0.143:  # 0.02
    ...
    def get_diffy_diffz(self, dy_dz_change_rows):
        ...
        #print("dy_change_row_avg[1]",dy_change_row_avg[1])
        #print("dy_change_row_avg[0]",dy_change_row_avg[0])
        #print("-------------------------")
        #time.sleep(5)
    ...
    def sequencing(self,  horizontal_handled_gridded_list, horizontal_plane_list, index, minz,minx,maxx):
        ...
        for ind, i in enumerate(horizontal_handled_gridded_list):
            if(horizontal_plane_list[ind]):
                ...
                if first_rows_taken:
                    ...
                    bottom_index_z_mod = self.bottom_index_row_offset*(abs(diff_dz_change) /
                                                      (abs(diff_dy_change)+abs(diff_dz_change)))


                    bottom_index_y_mod = self.bottom_index_row_offset*(abs(diff_dy_change) /
                                                  (abs(diff_dy_change)+abs(diff_dz_change)))
                                              
                    for dy_dz in bottom_dy_dz_change_rows[0]:
                        sign_dy_bottom = math.copysign(1, bottom_index_y_mod)
                        if sign_dy_bottom>0:
                            dy_dz[1] = dy_dz[1] + (bottom_index_y_mod*self.top_bottom_index_shifitng_factor)
                        else:
                            dy_dz[1] = dy_dz[1] - (bottom_index_y_mod*self.top_bottom_index_shifitng_factor)                            
                        dy_dz[2] = dy_dz[2] - (bottom_index_z_mod*self.top_bottom_index_shifitng_factor)
                ...
                if count <= skip_row_count:
                    if reaced_last_point:
                        ...
                        top2_index_z_mod = self.top_index_row_offset*(abs(diff_dz_change) /
                                                      (abs(diff_dy_change)+abs(diff_dz_change)))


                        top2_index_y_mod = self.top_index_row_offset*(abs(diff_dy_change) /
                                                      (abs(diff_dy_change)+abs(diff_dz_change)))
                        ...
                        for dy_dz in dy_dz_change_rows[1]:
                            ...
                            if sign_dy_top>0:
                                dy_dz[1] = dy_dz[1] + (top2_index_y_mod*self.top_bottom_index_shifitng_factor)
                            else:
                                dy_dz[1] = dy_dz[1] - (top2_index_y_mod*self.top_bottom_index_shifitng_factor)

                            dy_dz[2] = dy_dz[2] + (top2_index_z_mod*self.top_bottom_index_shifitng_factor)
        ...
        for ind_, row_ in enumerate(data_row_list):
                    ...
                    if minx < left_pt_x:
                        ...
                        diff_left_x = abs(left_pt_x - next_left_pt_x)
                        diff_left_y = abs(left_pt_y - next_left_pt_y)
                        #print("diff_left_x",diff_left_x)
                        #print("diff_left_y",diff_left_y)
                        percentage_x_change_left = ((diff_left_x)/(diff_left_x + diff_left_y))*100
                    ...
                    if maxx > right_pt_x:
                        ...
                        diff_right_x = abs(right_pt_x - next_right_pt_x)
                        diff_right_y = abs(right_pt_y - next_right_pt_y)
                        #print("diff_right_x",diff_right_x)
                        #print("diff_right_y",diff_right_y)
                        percentage_x_change_right = ((diff_right_x)/(diff_right_x + diff_right_y))*100
            ...
            for ind_pt, row_pt in enumerate(reversed(minx_maxx_data_row_list[:-2])):
                ...
                if (left_pt_x == minx) and (right_pt_x == maxx):
                    if incomplete_row_from_bottom_count>0:
                        ...
                        for ind_z,z_level in enumerate(incomplete_row_from_bottom_z_level):
                            for pt_z in extra_modified_bottom_row:
                                pt_z[2] = z_level
                            minx_maxx_data_row_list[-2-ind_z-1] = extra_modified_bottom_row
                                                            
                    bottom_row_z_level = bottom_row[0][2]
                    bottom_row_index_z_level = bottom_row_index[0][2]
                    modified_bottom_row = copy.deepcopy(row_pt)
                    for pt_ in modified_bottom_row:
                        pt_[2] = bottom_row_z_level
                    modified_bottom_row_index = copy.deepcopy(row_pt)
            ...
            for ind_pt, row_pt in enumerate(reversed(minx_maxx_data_row_list[2:])):
                ...
                if (left_pt_x == minx) and (right_pt_x == maxx):
                    if incomplete_row_from_top_count>0:
                        ...
                        for ind_z,z_level in enumerate(incomplete_row_from_top_z_level):
                            for pt_z in extra_modified_top_row:
                                pt_z[2] = z_level
                            minx_maxx_data_row_list[2+ind_z] = extra_modified_top_row
                                                            
                    top_row_z_level = top_row[0][2]
                    top_row_index_z_level = top_row_index[0][2]
                    modified_top_row = copy.deepcopy(row_pt)
                    for pt_ in modified_top_row:
                        pt_[2] = top_row_z_level
                        #pt_[1] = pt_[1] + top1_index_y_mod
                    modified_top_row_index = copy.deepcopy(row_pt)
        ...
        #print("minx_maxx_data_row_list",minx_maxx_data_row_list)
        #time.sleep(4)
        for all_pt in minx_maxx_data_row_list:
            for pts in all_pt:
                pts.append(False)
        ...
        ord_data_row_list = [None]*(len(order_list)+len(newly_arranged_data_row_list))
        ...
        for idx, row in enumerate(ord_data_row_list_combo):
            ...
            if row[0] == 'desc':
                ...
                left_point_added[0]= left_point_added[0] - (row_left_index_x_mod*self.left_right_index_shifitng_factor)
                if (left_point[1] - left_point_adj[1])>0:
                    left_point_added[1]= left_point_added[1] + (row_left_index_y_mod*self.left_right_index_shifitng_factor)
                else:
                    left_point_added[1]= left_point_added[1] - (row_left_index_y_mod*self.left_right_index_shifitng_factor)
                #print("before left_point[6]",row[1][0][-1])
                #left_point[6]= True
                new_data_row_list[idx][-1][6] = True
                #print("after left_point[6]",row[1][0][-1])

                right_point_added = copy.deepcopy(right_point)
                right_point_added[0]= right_point_added[0] + (row_right_index_x_mod*self.left_right_index_shifitng_factor)

                if (right_point[1] - right_point_adj[1])>0:
                    right_point_added[1]= right_point_added[1] + (row_right_index_y_mod*self.left_right_index_shifitng_factor)
                else:
                    right_point_added[1]= right_point_added[1] - (row_right_index_y_mod*self.left_right_index_shifitng_factor)
                #right_point_added[1]= right_point_added[1] + (row_right_index_y_mod*self.index_shifitng_factor)
                #right_point[6]= True
                new_data_row_list[idx][0][6] = True
                #right_point_added[6]=True
                #left_point_added[6]=True
        ...
        without_index_data_row_list_mod = [pt[:6] for row in without_index_row_list for pt in row]
        ...
        for ind, row in enumerate(new_data_row_list):
            ...
            for pt in row:
                ...
                data_row_point_append.is_index = pt[6]
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            if (self.received_sealer and self.received_base1 and self.received_base2 and self.received_clear1 and self.received_clear2):
                ...
                self.sealercoat_without_index_pc.publish(self.sealer_without_index_cloud)
                self.sealercoat_pc.publish(self.sealer_data_seq_cloud)

                self.basecoat1_without_index_pc.publish(self.base1_without_index_cloud)
                self.basecoat1_pc.publish(self.basecoat1_data_seq_cloud)
                
                self.basecoat2_without_index_pc.publish(self.base2_without_index_cloud)
                self.basecoat2_pc.publish(self.basecoat2_data_seq_cloud)

                self.clearcoat1_without_index_pc.publish(self.clear1_without_index_cloud)
                self.clearcoat1_pc.publish(self.clearcoat1_data_seq_cloud)
                
                self.clearcoat2_without_index_pc.publish(self.clear2_without_index_cloud)
...
for ind_, row_ in enumerate(data_row_list):
            ...
            if minx < left_pt_x:
                min_x_left_pt_x = row_[0]
                min_x_left_pt_x[0] = min_x
                minx_maxx_data_row_list[ind_].insert(0,min_x_left_pt_x)
                
            if maxx > right_pt_x:
                max_x_right_pt_x = row_[-1]
                max_x_right_pt_x[0] = max_x
                minx_maxx_data_row_list[ind_].append(max_x_right_pt_x)

                
            if float(width_current_row) > float(width_x)/2:
                if flage_dir:
                    print("Loop 1 skiped_row_id: ", skiped_row_id)
                    for id_,z_Val in zip(skiped_row_id,skiped_row_z):
                        for point_t in row_:
                            point_t[2] = z_Val
                        filled_data_row_list.insert(id_,row_)
                        last_added_row = row_
                    skiped_row_id= []
                    flage_dir = False
                    print("loop 1")
                else:
                    filled_data_row_list.append(row_)
                    last_added_row = row_
                    print("loop 2")

            elif float(width_current_row) < float(width_x)/2:
                print("loop 3")
                flage_dir = True
                skiped_row_id.append(ind_)
                skiped_row_z.append(row_[0][2])
                
            if flage_dir and ind_ == len(data_row_list)-1:
                print("Loop 4 skiped_row_id: ", skiped_row_id)
                print("Prev last_added_row", last_added_row)
                print("Prev skiped_row_z", skiped_row_z)
                for id_,z_Val in zip(skiped_row_id,skiped_row_z):
                    print("before len filled_data_row_list",len(filled_data_row_list))
                    row_add_last = []
                    pt_mod = []
                    for point_t in last_added_row:
                        pt_mod = point_t
                        pt_mod[2] = z_Val
                        row_add_last.append(pt_mod)
                    print("modify z_Val",z_Val)
                    print("from loop Prev last_added_row", last_added_row) 
                    
                    filled_data_row_list.append(row_add_last)        
                         
                    print("after len filled_data_row_list",len(filled_data_row_list))
                    
                    filled_data_row_list_update = filled_data_row_list
                    #last_added_row = row_
        ...
        print("filled_data_row_list",filled_data_row_list)   
        #filled_data_row_list = data_row_list'''
