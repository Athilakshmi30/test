...
class SyntheticCloud():
    ...
    def createGrid(self, cloud):
        ...
        f = open("gridded_cloud.txt", "w")
        f.write(str(gridded_cloud))
        f.close()
        print("writng done ----------------------------------------------------------------------------------------------------------------------------------")
            ...
            for pt in row:
                ...
                for ind,xcoord in enumerate(X_data):
                    ...
                    if(abs(xcoord - pt[0]) < 0.001 and abs(Y_data[ind] - pt[1]) < 0.001 and abs(Z_data[ind] - pt[2]) < 0.001):
                        fitted_points.append([xcoord,fit_equation(xcoord),z_val,pt[3],pt[4],pt[5]])
    ...
    def sequencing(self, data, index, minz):
        ...
        for row in data_row_list:
            ...
            row_reverse= reversed(row)
            data_reverse.append(row_reverse)
