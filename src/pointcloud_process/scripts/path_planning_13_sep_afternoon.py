...
class PathPlanning():
    ...
    def __init__(self):
        ...
        self.left_right_index_offset = 0.0254 #0.0762  # 3 inches #0.1524 - 6 inches
            ...
            if(horizontal_plane_list[ind]):
                ...
                if count <= skip_row_count:
                    if reaced_last_point:
                        ...
                        index_z_mod = diff_dz_change
                        ##index_z_mod = (index+1)*0.01*(abs(diff_dz_change) /
                        ##                              (abs(diff_dy_change)+abs(diff_dz_change)))
                        
                        index_y_mod = diff_dy_change
                        ...
                        top_index_z_mod = self.top_index_row_offset*(abs(diff_dz_change) /
                                                      (abs(diff_dy_change)+abs(diff_dz_change)))


                        top_index_y_mod = self.top_index_row_offset*(abs(diff_dy_change) /
                                                      (abs(diff_dy_change)+abs(diff_dz_change)))
                        ...
                        for dy_dz in dy_dz_change_rows[1]:
                            sign_dy_top = math.copysign(1, top_index_y_mod)
                            if sign_dy_top>0:
                                dy_dz[1] = dy_dz[1] + (top_index_y_mod*self.top_bottom_index_shifitng_factor)
                            else:
                                dy_dz[1] = dy_dz[1] - (top_index_y_mod*self.top_bottom_index_shifitng_factor)

                            dy_dz[2] = dy_dz[2] + (top_index_z_mod*self.top_bottom_index_shifitng_factor)
